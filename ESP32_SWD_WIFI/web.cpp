...
#define str(a) #a
  ...
  server.on("/set_width", HTTP_POST, [](AsyncWebServerRequest *request) {
    if (request->hasParam("width_start") && request->hasParam("width_max")) {
      int new_width_start = request->getParam("width_start")->value().toInt();
      int new_width_max = request->getParam("width_max")->value().toInt();
      request->send(200, "text/plain", "Ok set width: " + String(new_width_start) + " width_max: " + String(new_width_max));
      set_width(new_width_start, new_width_max);
      return;
    }
    request->send(200, "text/plain", "Wrong parameter");
  });
